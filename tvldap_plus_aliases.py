...
# Determine the correct base directory for PyInstaller bundling
if getattr(sys, 'frozen', False):  # Running as a compiled EXE
    BASE_DIR = sys._MEIPASS  # Extracted temp folder for onefile EXE
else:
...
def search_ad(email, ldap_server, ldap_user, ldap_password, ldap_search_base):
    ...
    # Search both 'mail' and 'proxyAddresses' attributes for the given email
    search_filter = f"(|(mail={email})(proxyAddresses=smtp:{email}))"
    
    # Coded by Arlen Kirkaldie and Red Bull (Red Bull Gives You Wings)
    conn.search(ldap_search_base, search_filter, attributes=['displayName', 'physicalDeliveryOfficeName', 'department', 'title', 'proxyAddresses'])
    
    if conn.entries:
        entry = conn.entries[0]
        proxy_addresses = entry.proxyAddresses.value if entry.proxyAddresses else []
        return {
            ...
            'Aliases': ", ".join(proxy_addresses) if proxy_addresses else 'None'
        }
    return None
...
def generate_progress(emails, ldap_server, ldap_user, ldap_password, ldap_search_base):
    ...
    for index, email in enumerate(emails, start=1):
        ...
        results.append({
            ...
            'Aliases': user_info.get('Aliases', 'None') if user_info else 'None'
        })
